...
/**
 * Check that a chosen pickup date falls within the Mon–Sun week of pickup_monday.
 */
function isDateInPickupWeek(pickupDate: string, pickupMonday: string): boolean {
  const chosen = new Date(`${pickupDate}T00:00:00Z`)
  const monday = new Date(`${pickupMonday}T00:00:00Z`)
  if (Number.isNaN(chosen.getTime()) || Number.isNaN(monday.getTime())) return false

  const sunday = new Date(monday)
  sunday.setUTCDate(monday.getUTCDate() + 6)

  return chosen >= monday && chosen <= sunday
}

// PATCH /api/chickens/orders/[id]/pickup-date
export async function PATCH(
  ...
  try {
    ...
    const { data: order, error: orderError } = await supabaseAdmin
      .from('chicken_orders')
      .select('id, status, pickup_monday, user_id, customer_email, customer_phone')
      .eq('id', params.id)
      .single()
    ...
    // Validate date is within the pickup week (Mon–Sun)
    if (!isDateInPickupWeek(pickupDate, order.pickup_monday)) {
      return NextResponse.json(
        { error: 'Pickup date must be within the pickup week' },
        { status: 400 }
      )
    }
