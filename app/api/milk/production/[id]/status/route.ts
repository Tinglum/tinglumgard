...
import { advanceProductionStatus, ProductionError } from '@/lib/milk/production'
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const batch = await advanceProductionStatus(params.id, body, access.session)
    return NextResponse.json({ batch })
  } catch (error: any) {
    if (error instanceof ProductionError) return NextResponse.json({ error: error.message }, { status: error.status })
    return NextResponse.json({ error: error?.message || 'Failed to advance' }, { status: 500 })
  }
}
