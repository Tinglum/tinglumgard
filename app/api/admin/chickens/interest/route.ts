...
export async function GET(_request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      logError('admin-chicken-interest-fetch', error);
    ...
    logError('admin-chicken-interest-get', error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    switch (action) {
      case 'contact':
        return await contactChickenInterest(interestIds);
      case 'convert':
        return await convertChickenInterest(interestIds);
      default:
        return NextResponse.json({ error: 'Unknown action' }, { status: 400 });
    }
  } catch (error) {
    logError('admin-chicken-interest-post', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

async function contactChickenInterest(interestIds: string[]) {
  ...
  if (error || !entries) {
    logError('admin-chicken-interest-contact-fetch', error);
  ...
  for (const entry of entries) {
    ...
    const orderLink = `${appUrl}/kylling/bestill`;

    try {
      const rendered = await renderManagedTemplate({
        templateKey: 'chicken.interest.season_open',
        locale: 'no',
        variables: {
      ...
      const subject = rendered?.subject ?? `Bestilling åpnet – ${breedName}`;
      const html = rendered?.html ?? buildChickenInterestContactHtml({ customerName, breedName, orderLink });
      ...
      await dispatchEmail({
        ...
        templateKey: rendered?.templateKey ?? 'chicken.interest.season_open',
        sourcePath: '/api/admin/chickens/interest',
      });
      ...
      logError('admin-chicken-interest-contact-email', emailError);
...
async function convertChickenInterest(interestIds: string[]) {
  ...
  if (error) {
    logError('admin-chicken-interest-convert', error);
...
function buildChickenInterestContactHtml(params: {
  customerName: string;
  breedName: string;
  orderLink: string;
}): string {
...
<div class="title">Bestilling åpnet – ${params.breedName}</div>
<p>Hei ${params.customerName},</p>
<p>Sesongen er åpnet og du kan nå bestille <strong>${params.breedName}</strong>.</p>
