...
export async function POST(
  ...
  const { data: order, error: orderError } = await supabaseAdmin
    .from('chicken_orders')
    .select('id, order_number, customer_name, customer_email, delivery_method, pickup_week, pickup_monday')
    .eq('id', params.id)
    .maybeSingle();
  ...
  const orderUrl = buildCustomerOrderLink(appUrl, 'chicken', String(order.id));
  const firstName = String(order.customer_name || 'Kunde').split(/\s+/)[0];

  const pickupWeekLabel = order.pickup_monday
    ? new Intl.DateTimeFormat('nb-NO', { day: 'numeric', month: 'long', year: 'numeric' }).format(
        new Date(order.pickup_monday + 'T12:00:00Z')
      )
    : `uke ${order.pickup_week}`;
...
<html lang="no">
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f4f4f5;padding:24px 0;"><tr><td align="center">
      <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="width:100%;max-width:600px;background:#ffffff;border:1px solid #e4e4e7;border-radius:14px;overflow:hidden;">
        ...
        <tr><td style="padding:24px;font-family:Arial,sans-serif;font-size:16px;line-height:1.6;color:#111827;">
          <p>Hei ${firstName},</p>
          <p>Henteuken for kyllingbestilling <strong>${order.order_number}</strong> nærmer seg (${pickupWeekLabel}).</p>
          <p>Gå inn på Min side og velg hentedag og tidspunkt (kl. 11 eller 17), slik at vi er klare for deg.</p>
          <p><a href="${orderUrl}" style="display:inline-block;background:#111827;color:#ffffff;text-decoration:none;padding:10px 16px;border-radius:8px;">Velg hentedag og tid</a></p>
  ...
  await supabaseAdmin.from('email_dispatch_queue').insert({
    idempotency_key: `email:admin-manual:chicken_order:${order.id}:chicken.pickup.choose_day:${Date.now()}`,
    ...
    subject,
    html,
    metadata: { flow_key: 'chicken.choose_pickup_day', manual_send: true, product_scope: 'chickens' },
    source_path: '/api/admin/chickens/orders/[id]/send-pickup-email',
