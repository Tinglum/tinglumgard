-- ================================================
-- REFERRAL SYSTEM MIGRATION (No Users Table)
-- ================================================
-- Modified version that works without a users table
-- Uses phone numbers as user identifiers instead
...
CREATE TABLE IF NOT EXISTS referral_codes (
  ...
  -- User identification (using phone instead of user_id)
...
CREATE INDEX idx_referral_codes_owner_phone ON referral_codes(owner_phone);
...
-- ================================================
-- 5. FUNCTION: Check if customer is new
-- ================================================
...
BEGIN
  ...
  FROM orders
  WHERE
    (customer_phone = p_phone OR customer_email = p_email)
...
BEGIN
  ...
  IF v_code_record.owner_phone = p_phone THEN
    RETURN jsonb_build_object(
      'valid', FALSE,
  ...
  v_is_new_customer := is_new_customer(p_phone, p_email);
  ...
  RETURN jsonb_build_object(
    ...
    'referrer_phone', v_code_record.owner_phone,
...
-- GRANT PERMISSIONS
-- ================================================
GRANT SELECT, INSERT, UPDATE ON referral_codes TO authenticated, anon;
GRANT SELECT, INSERT, UPDATE ON referrals TO authenticated, anon;
GRANT EXECUTE ON FUNCTION is_new_customer TO authenticated, anon;
GRANT EXECUTE ON FUNCTION validate_referral_code TO authenticated, anon;
