...
export async function sendViaMailgun({
  ...
  const fromAddress = from || process.env.EMAIL_FROM || 'post@tinglum.com';
  const replyAddress = replyTo || process.env.EMAIL_REPLY_TO || fromAddress;
  ...
  try {
    ...
    console.error('Mailgun send exception:', { error, to, subject });
