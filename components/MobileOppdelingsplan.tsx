  ...
  return (
    <div className="space-y-6 pb-20">
      <header className="rounded-2xl border border-[#E6D8C8] bg-[#FFF9F2] p-6 shadow-[0_12px_30px_rgba(50,36,24,0.08)]">
        <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[#6C5A4A]">{t.oppdelingsplan.title}</p>
        <h1 className="mt-2 text-3xl font-bold text-[#1F1A14]">{t.oppdelingsplan.title}</h1>
        <p className="mt-2 text-sm text-[#6C5A4A]">{t.oppdelingsplan.subtitle}</p>
      </header>
      ...
      <div className="rounded-2xl border border-[#E6D8C8] bg-white p-4">
        ...
        <p className="mt-3 text-xs text-[#6C5A4A]">{t.oppdelingsplan.clickForInfo}</p>
      </div>

      <div className="grid gap-4">
        <div className="rounded-2xl border border-[#E6D8C8] bg-white p-4">
          <div className="flex items-center gap-2 text-sm font-semibold text-[#1F1A14]">
            <Check className="h-4 w-4 text-[#2F5D3A]" />
            {t.oppdelingsplan.inBox}
          </div>
          <ul className="mt-3 space-y-2 text-sm text-[#6C5A4A]">
            {inBoxSummary.map(item => (
              <li key={item} className="flex items-start gap-2">
                <span className="mt-2 h-1.5 w-1.5 rounded-full bg-[#2F5D3A]" />
                <span>{item}</span>
              </li>
            ))}
          </ul>
        ...
        <div className="rounded-2xl border border-[#E6D8C8] bg-white p-4">
          <div className="flex items-center gap-2 text-sm font-semibold text-[#1F1A14]">
            <Plus className="h-4 w-4 text-[#C05621]" />
            {t.oppdelingsplan.canOrder}
          </div>
          <ul className="mt-3 space-y-2 text-sm text-[#6C5A4A]">
            {canOrderSummary.map(item => (
              <li key={item} className="flex items-start gap-2">
                <span className="mt-2 h-1.5 w-1.5 rounded-full bg-[#C05621]" />
                <span>{item}</span>
              </li>
            ))}
          </ul>
      ...
      <div className="space-y-3">
        {cuts.map((cut) => {
          ...
          return (
            <div key={cut.id} className="rounded-2xl border border-[#E6D8C8] bg-white">
              <button
                onClick={() => setExpandedCut(isOpen ? null : cut.id)}
                className="flex w-full items-center justify-between px-4 py-4 text-left"
              >
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[#6C5A4A]">Del {cut.id}</p>
                  <p className="mt-1 text-lg font-semibold text-[#1F1A14]">{cut.name}</p>
                </div>
                <ChevronDown className={`h-5 w-5 text-[#6C5A4A] transition-transform ${isOpen ? 'rotate-180' : ''}`} />
              </button>

              {isOpen && (
                <div className="px-4 pb-4 text-sm text-[#6C5A4A]">
                  <p className="mb-3">{cut.description}</p>
                  <div className="grid gap-3">
                    <div className="rounded-xl border border-[#E6D8C8] bg-[#FFF9F2] p-3">
                      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#6C5A4A]">{t.oppdelingsplan.inBox}</p>
                      {cut.inBox.length > 0 ? (
                    ...
                    <div className="rounded-xl border border-[#E6D8C8] bg-[#FFF9F2] p-3">
                      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#6C5A4A]">{t.oppdelingsplan.canOrder}</p>
